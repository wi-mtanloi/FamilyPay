...
export const HomeTab: React.FC<HomeTabProps> = ({
  ...
  const handlePayBill = async (bill: FixedBill) => {
    if (wallets.length === 0) {
    ...
    const confirm = window.confirm(`Bạn có muốn thanh toán hóa đơn "${bill.name}" trị giá ${formatVND(bill.amount)} từ "${defaultWallet.name}"?`);
    if (!confirm) return;
    ...
    try {
      ...
      const newTx = {
        ...
        amount: bill.amount,
        type: 'expense',
        category: bill.category || 'Nhà cửa & Tiền ích',
        walletId: defaultWallet.id,
        walletName: defaultWallet.name,
        member: currentUserRole,
        notes: `Thanh toán hóa đơn cố định: ${bill.name}`,
        createdBy: currentUserUid
      };
      ...
      const newBalance = defaultWallet.balance - bill.amount;
      await updateFamilyItem(familyId, 'wallets', defaultWallet.id, { balance: newBalance });

      // Update last paid date on the bill
      await updateFamilyItem(familyId, 'bills', bill.id, {
        lastPaidDate: new Date().toISOString().split('T')[0]
      });

      alert(`Thanh toán thành công hóa đơn ${bill.name}!`);
    } catch (err: any) {
